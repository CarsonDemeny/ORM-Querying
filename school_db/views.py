...
def problem_four(request):
  print(f'Students Counts: {Student.objects.count()}')
  print(f'Courses Count: {Course.objects.count()}')
  print(f'Instructors Count: {Instructor.objects.count()}')
...
def problem_five(request):
  new_student = Student.objects.create(first_name = 'Carson', last_name = 'Demeny', year = '2022', gpa = 3.6)
...
def bonus_problem(request):
  instructors = Instructor.objects.annotate(number_courses_teaching = Count('course')).filter(number_courses_teaching = 1)
  for instructor in instructors:
    print(f'Full Name: {instructor.first_name} {instructor.last_name} \n Courses Teaching: {instructor.number_courses_teaching}')
